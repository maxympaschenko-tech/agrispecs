...
export async function GET(){
  try{
    const [migrationApplied,machineRows,currentVersions,specificationRows,grossPowerRows,ptoRows,loaderRows,loaderFitments,serviceParts,versionedServiceFitments,hydraulicSupersessionRows]=await Promise.all([
      count(`SELECT COUNT(*) AS count FROM schema_migrations WHERE id='20260827_162_kubota_l4802_hydraulic_filter_supersession'`),
      ...
      count(`SELECT COUNT(*) AS count FROM machine_attachments ma JOIN machines m ON m.id=ma.machine_id JOIN manufacturers mf ON mf.id=m.manufacturer_id JOIN attachments a ON a.id=ma.attachment_id WHERE mf.slug='kubota' AND m.slug='l4802' AND a.slug='la766' AND ma.confidence='official'`),
    ...
    return NextResponse.json({
      ok,
      expectedLatestL4802Migration:'20260827_162_kubota_l4802_hydraulic_filter_supersession',
      checks,
      values:{machineRows,currentVersions,specificationRows,grossPowerRows,ptoRows,loaderRows,loaderFitments,serviceParts,versionedServiceFitments,hydraulicSupersessionRows},
      expected:{machineRows:1,currentVersions:3,specificationRows:90,grossPowerRows:3,ptoRows:3,loaderRows:1,loaderFitments:1,serviceParts:6,versionedServiceFitments:18,hydraulicSupersessionRows:1},
    },{
      status:ok?200:503,
