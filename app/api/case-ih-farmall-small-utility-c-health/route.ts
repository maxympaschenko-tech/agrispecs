...
export async function GET(){
 try{
  ...
  const migrations=await count(db,`SELECT COUNT(*) n FROM schema_migrations WHERE id IN (?,?)`,migrationIds);
 ...
 }catch(error){console.error(error);return NextResponse.json({ok:false,error:'Farmall Small Utility C health check failed'},{status:500});}
}
