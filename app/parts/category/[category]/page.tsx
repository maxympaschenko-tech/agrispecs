...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${category.name} - Farm Equipment Parts & Compatibility`,
    description: descriptionFor(category.slug, category.name),
    alternates: { canonical: `/parts/category/${category.slug}` },
    robots: indexable ? { index: true, follow: true } : { index: false, follow: true },
  };
}

export default async function PartCategoryPage({ params }: PageProps) {
  const { category: slug } = await params;
  const category = await getPartCategory(slug);
  if (!category) notFound();

  const parts = await getPartCatalogItems(category.slug);
      ...
      <section className="section" style={{ paddingTop: 18 }}>
        <div className="container">
          ...
          <div className="parts-stats">
            ...
            <div><strong>{kitLinked}</strong><span>with verified kit relationships</span></div>
          </div>
