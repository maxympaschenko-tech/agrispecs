...
const models:ModelSeed[]=[
  {slug:'8rx-310',model:'8RX 310',ratedHp:310,maxHp:341,ptoHp:257,aspiration:'Single variable-geometry turbocharger'},
  {slug:'8rx-340',model:'8RX 340',ratedHp:340,maxHp:374,ptoHp:284,aspiration:'Single variable-geometry turbocharger'},
  {slug:'8rx-370',model:'8RX 370',ratedHp:370,maxHp:407,ptoHp:310,aspiration:'Dual turbochargers - variable-geometry plus fixed-geometry in series'},
  {slug:'8rx-410',model:'8RX 410',ratedHp:410,maxHp:443,ptoHp:310,aspiration:'Dual turbochargers - variable-geometry plus fixed-geometry in series'},
];
const definitions=[
...
async function selectId(connection:Parameters<DbMigration['apply']>[0],sql:string,params:unknown[]=[]){const [rows]=await connection.query<IdRow[]>(sql,params);if(!rows[0])throw new Error('Missing John Deere 8RX migration dependency.');return Number(rows[0].id);}
...
export const johnDeere8RXCurrentSpecsMigration:DbMigration={
  id:'20260828_217_john_deere_8rx_current_specs',
  description:'Add current US John Deere 8RX 310-410 four-track tractors with official JD9 power and PTO specifications',
  async apply(connection){
    ...
    await connection.query(`INSERT INTO machine_series (manufacturer_id,equipment_type_id,name,slug) VALUES (?,?,'8RX Series','8rx-series') ON DUPLICATE KEY UPDATE name='8RX Series'`,[manufacturerId,equipmentTypeId]);const seriesId=await selectId(connection,`SELECT id FROM machine_series WHERE manufacturer_id=? AND equipment_type_id=? AND slug='8rx-series' LIMIT 1`,[manufacturerId,equipmentTypeId]);
