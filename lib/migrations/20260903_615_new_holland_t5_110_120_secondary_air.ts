...
const PART_NUMBER = '87720899';
const FITMENT_URL = 'https://www.messicks.com/parts/new-holland/87720899';
const FITMENT_EXTERNAL_ID = 'messicks-t5-110-120-stagev-87720899-secondary-air-2026-09';
...
async function selectId(connection: Parameters<DbMigration['apply']>[0], sql: string, params: unknown[] = []) {
  ...
  if (!rows[0]) throw new Error('Missing T5.110/T5.120 secondary-air migration dependency.');
...
export const newHollandT5110120SecondaryAirMigration: DbMigration = {
  id: '20260903_615_new_holland_t5_110_120_secondary_air',
  description: 'Add exact T5.110/T5.120 AutoCommand and Dynamic Command Stage V fitment for secondary engine-air filter 87720899',
  async apply(connection) {
    ...
    const sourceRecordId = await ensureSourceRecord(
      ...
      FITMENT_EXTERNAL_ID,
      FITMENT_URL,
      `Messick's New Holland ${PART_NUMBER} T5.110/T5.120 Stage V secondary-air catalog`,
      {
        ...
        evidence: "Messick's lists 87720899 in maintenance-parts/filter and air-cleaner component paths for T5.110 and T5.120 AutoCommand and Dynamic Command Stage V North American catalog families.",
        replacementHistory: 'The 51447248 and 87687268 predecessor relationships are already recorded separately by migration 611 and are not duplicated here.',
        confidence: 'secondary/high',
        guardrail: 'This migration adds only current-part fitment for the exact AutoCommand/Dynamic Command Stage V families. It does not copy fitment to predecessor part numbers or infer ElectroCommand.',
      },
    );
    ...
    for (const model of models) {
      ...
      for (const configuration of configurations) {
        const fitmentNote = `${PART_NUMBER} Secondary Engine Air Filter is listed in the exact ${model.model} ${configuration.key} Stage V North American maintenance and air-cleaner catalogs. Confirm transmission and air-cleaner configuration before ordering.`;
