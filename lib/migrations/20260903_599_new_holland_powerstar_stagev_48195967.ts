...
const SOURCE_EXTERNAL_ID = 'messicks-powerstar-stagev-48195967-2026-09';
const OFFICIAL_URL = 'https://www.mycnhstore.com/gb/en/newhollandag/category/filters/hydraulic-filter/hydraulic-oil-filter/p/48195967';
const OFFICIAL_EXTERNAL_ID = 'new-holland-mycnh-48195967-powerstar-identity-2026-09';
...
const fitments = [
  ...
  },
  {
    slug: 'powerstar-120',
    model: 'PowerStar 120',
    configuration: 'Dual Command 1.5, Stage V, cab, MY24 catalog (07/24)',
  },
] as const;

async function selectId(connection: Parameters<DbMigration['apply']>[0], sql: string, params: unknown[] = []) {
  const [rows] = await connection.query<IdRow[]>(sql, params);
  if (!rows[0]) throw new Error('Missing PowerStar Stage V 48195967 migration dependency.');
...
export const newHollandPowerStarStageV48195967Migration: DbMigration = {
  id: '20260903_599_new_holland_powerstar_stagev_48195967',
  description: 'Add exact Stage V PowerStar 90/100/110/120 fitment for hydraulic filter 48195967 on the next-generation current version',
  async apply(connection) {
    ...
    await connection.query(
      ...
      [manufacturerId, hydraulicCategoryId, PART_NUMBER, PART_NUMBER, 'Hydraulic Oil Filter'],
    );
    ...
    const fitmentSourceRecordId = await ensureSourceRecord(
      ...
      SOURCE_EXTERNAL_ID,
      SOURCE_URL,
      `Messick's New Holland ${PART_NUMBER} Stage V PowerStar fitment catalog`,
      {
        ...
        catalogEvidence: 'Messick\'s part catalog lists 48195967 in Stage V PowerStar 90/100/110/120 Dual Command MY24 maintenance/steering-filter paths; PowerStar 100/110/120 also expose MY25 Power Shuttle hydraulic-pump/filter paths.',
        confidence: 'secondary/high',
        guardrail: 'Only the explicit Dual Command MY24 configurations are inserted here. PowerStar 75 and other configurations are not inferred.',
      },
    );
    ...
    await ensureSourceRecord(
      ...
      OFFICIAL_URL,
      `New Holland MyCNH ${PART_NUMBER} Hydraulic Oil Filter`,
      {
    ...
    for (const fitment of fitments) {
      ...
      const fitmentNote = `Hydraulic/steering-system filter ${PART_NUMBER} is listed in the exact ${fitment.model} ${fitment.configuration} Stage V catalog. Confirm transmission and hydraulic configuration before ordering.`;
