...
const URL = 'https://www.caseih.com/en-us/unitedstates/products/tractors/puma-super-series/puma-series-new/puma-155';
  ...
  async apply(c) {
    ...
    await c.query(
      `INSERT INTO machine_series(manufacturer_id,equipment_type_id,name,slug)
       VALUES(?,?,'Puma Series (New)','puma-series-new')
       ON DUPLICATE KEY UPDATE name=VALUES(name)`,
      [mf, et],
    );
    const series = await id(
      c,
      `SELECT id FROM machine_series WHERE manufacturer_id=? AND equipment_type_id=? AND slug='puma-series-new' LIMIT 1`,
      [mf, et],
    );
    ...
    if (!sourceRecordId) {
      const [result] = await c.query<ResultSetHeader>(
        ...
        [
          ...
          URL,
          EXTERNAL_ID,
          'Case IH US Puma 155 New official current product specifications',
          JSON.stringify({ model: 'Puma 155 New', horsepower: 155, engineDisplacementL: 6.7 }),
        ],
      );
    ...
    for (const d of [
      ...
      await c.query(
        `INSERT INTO spec_definitions(section,spec_key,label,value_type,canonical_unit,display_order)
         VALUES(?,?,?,?,?,?)
         ON DUPLICATE KEY UPDATE label=VALUES(label),value_type=VALUES(value_type),canonical_unit=VALUES(canonical_unit)`,
        d,
      );
    }
    ...
    await c.query(
      `INSERT INTO machines(manufacturer_id,equipment_type_id,series_id,model_name,slug,market_notes,data_status)
       VALUES(?,?,?,?,?,'Current US Case IH Puma Series (New) tractor','partial')
       ON DUPLICATE KEY UPDATE series_id=VALUES(series_id),model_name=VALUES(model_name),market_notes=VALUES(market_notes)`,
      [mf, et, series, 'Puma 155 New', 'puma-155'],
    );
    const machineId = await id(c, `SELECT id FROM machines WHERE manufacturer_id=? AND slug='puma-155' LIMIT 1`, [mf]);
    ...
    await c.query(
      `INSERT INTO machine_versions(machine_id,slug,market_code,market_name,configuration,is_current,source_record_id,notes)
       VALUES(?,?,'US','United States','Current Puma Series (New) tractor',TRUE,?,'Official current Case IH US product page. PTO power is intentionally omitted because the accessible current page does not publish it.')
    ...
    for (const [key, value, unit] of [
      ...
      await c.query(
        ...
        [machineId, versionId, definitionId, value, unit, sourceRecordId],
      );
    }
    ...
    await c.query(
      ...
      [machineId, l116, 'Official Case IH L116 compatibility includes Puma 155.', l116Source],
    );
  },
};
